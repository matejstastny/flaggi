...
public class LobbyInviteScreen extends Renderable implements Scrollable, Interactable {
	...
	public LobbyInviteScreen(LobbyHandler handler) {
		super(ZIndex.MENU_SCREEN, PanelRegion.BACKGROUND, UiTags.LOBBY);
	...
	public void render(Graphics2D g, int[] viewportOffset, Container focusCycleRootAncestor) {
		...
		g.fillRect(0, 0, px(100), px(100));

		// Header
		g.setColor(new Color(50, 50, 50));
		g.fillRect(0, 0, px(100), 70);
		g.setColor(Color.WHITE);
		g.setFont(new Font("SansSerif", Font.BOLD, 24));
		g.drawString("Online players", 20, 45);

		updateMaxScroll(new int[] { px(100), px(100) });
		int yOffset = scrollOffset + 80;

		// Render client items
		for (ClientItem item : clientItems) {
			renderClientItem(g, item, yOffset, new int[] { px(100), px(100) });
