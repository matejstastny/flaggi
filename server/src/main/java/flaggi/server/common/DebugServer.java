 ...
 *
 * <p>Usage: DebugServer.start(); // call once on server boot DebugServer.publish(updates,
 * clientInputs); // call every game tick DebugServer.stop(); // call on shutdown
 */
public class DebugServer {
     ...
     *
     * @param updates map of playerUuid → ServerStateUpdate (one per player, "me" is that player's
     *     object)
     * @param clientInputs map of playerUuid → latest ClientStateUpdate received from that client
     * @param tick current server tick number
     */
    public static void publish(
            Map<String, ServerStateUpdate> updates, Map<String, ClientStateUpdate> clientInputs, int tick) {
    ...
    private static String buildJson(
        ...
        for (Map.Entry<String, ServerStateUpdate> entry : updates.entrySet()) {
            ...
            sb.append("\"animation\":")
                    .append(jsonStr(me.getAnimation().name()))
                    .append(",");
            ...
            if (input != null) {
                sb.append("\"mouse\":{\"x\":")
                        .append(input.getClientMouseInput().getX())
                        .append(",\"y\":")
                        .append(input.getClientMouseInput().getY())
                        .append("},");
                sb.append("\"keys\":[");
