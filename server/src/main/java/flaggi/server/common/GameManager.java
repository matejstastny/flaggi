...
public class GameManager implements Closeable, Updatable {
    ...
    private void spawnBullet(String ownerUuid, PlayerGameObject player, int mouseX, int mouseY) {
        BulletGameObject bullet =
                new BulletGameObject(player.x(), player.y(), mouseX, mouseY, ownerUuid, Hitboxes.bullet());
    ...
    private void tickBullets() {
        ...
        for (BulletGameObject bullet : gameData.getBullets()) {
            ...
            // --- Hit player? -----------------------------------------------------
            for (Map.Entry<String, PlayerGameObject> entry :
                    gameData.getPlayers().entrySet()) {
                String targetUuid = entry.getKey();
                PlayerGameObject target = entry.getValue();

                // Can't shoot yourself loll
                if (targetUuid.equals(bullet.ownerUuid())) continue;
     ...
     */
    private void resolvePickupOrReturn(
            String playerUuid, PlayerGameObject player, Team playerTeam, FlagGameObject flag) {
        if (!player.overlaps(flag)) return;
     ...
     */
    private void moveCarriedFlag(
            String playerUuid, PlayerGameObject player, Team playerTeam, FlagGameObject carriedFlag) {
        if (!carriedFlag.carrierUuid().equals(playerUuid)) return;
        ...
        for (FlagGameObject baseFlag : gameData.getFlags()) {
            if (baseFlag == carriedFlag) continue; // same object
            if (baseFlag.team() != playerTeam) continue; // must be own base flag
            if (!baseFlag.isAtBase()) continue; // base flag must still be home
            ...
            if (player.overlaps(baseFlag)) {
                ...
                Logger.log(
                        LogLevel.DBG,
                        "Player " + playerUuid + " CAPTURED the flag! Score: " + gameData.getScore(playerUuid));
    ...
    private void endGame(String winnerUuid, String winnerName) {
        ...
        ServerMessage msg = ServerMessage.newBuilder()
                .setServerEndGame(ServerEndGame.newBuilder()
    ...
    private void sendJoinGameMessages(int roomWidth, int roomHeight) {
        Logger.log(
                LogLevel.DBG,
                "Sending room size [" + roomWidth + "," + roomHeight + "] to clients for game UUID: " + gameUuid);
        for (Client client : clients) {
            client.sendMessage(ServerMessage.newBuilder()
                    .setServerJoinGame(ServerJoinGame.newBuilder()
    ...
    private MapData getRandomMap() {
        ...
        try (BufferedReader br = new BufferedReader(new InputStreamReader(
                getClass().getClassLoader().getResourceAsStream(Constants.MAPS_RES_DIR + "/" + file)))) {
            String line;
