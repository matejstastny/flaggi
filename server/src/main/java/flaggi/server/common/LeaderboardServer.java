  ...
  private static void handleLeaderboard(io.javalin.http.Context ctx) {
    ...
    if (players.isEmpty()) {
      ...
      for (int i = 0; i < players.size(); i++) {
        ...
        String rankClass =
            rank == 1
                ? "rank rank-1"
                : rank == 2 ? "rank rank-2" : rank == 3 ? "rank rank-3" : "rank";
        String rankLabel =
            rank == 1 ? "🥇" : rank == 2 ? "🥈" : rank == 3 ? "🥉" : String.valueOf(rank);
    ...
    if (recentGames.isEmpty()) {
      ...
      for (GameHistoryEntry g : recentGames) {
        ...
        String winnerLabel =
            g.winnerTeam.substring(0, 1).toUpperCase() + g.winnerTeam.substring(1) + " Team";
