...
public class Constants {
    ...
    public static final String LOG_FILE = String.join(
            File.separator, FileUtil.getApplicationDataFolder(), Constants.APP_DATA_DIR_NAME, "logs", "latest.log");
    public static final ConfigManager CONFIG = getConfigManager();

    /**
     * Resources that will be created at server start in the directory the sever lives. The map is in
     * "/resource/path" "new relative path"
     */
    ...
    public static final int SERVER_SHUTDOWN_TIMEOUT_SEC = 1;
    ...
    private static ConfigManager getConfigManager() {
        try {
            return new ConfigManager(
                    String.join(File.separator, FileUtil.getJarExecDirectory(), "config.properties"),
    ...
    private static int getConfigIntValue(String key) {
        ...
        try {
            ...
            Logger.log(
                    LogLevel.ERR,
                    "Configuration error: The field \""
                            + key
                            + "\" is assigned a value of the wrong type. Expected: Integer.");
            Server.handleFatalError();
        }
        return val;
    }
