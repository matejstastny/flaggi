...
public class GameManager implements Closeable, Updatable {
  ...
  public void update() {
    ...
    ClientStateUpdate outgoing =
        ClientStateUpdate.newBuilder()
            .setPlayerUuid(uuid)
            .setGameUuid(gameUuid)
            .setClientMouseInput(
                ClientMouseInput.newBuilder().setX(worldMouseX).setY(worldMouseY).build())
            .setClientKeyInput(ClientKeyInput.newBuilder().addAllHeldKeys(keySnapshot).build())
            .build();
  ...
  private void onKeyPressed(KeyEvent e) {
    ...
    Set<ClientKey> updated =
        EnumSet.copyOf(heldKeys.isEmpty() ? EnumSet.noneOf(ClientKey.class) : heldKeys);
    updated.add(key);
    heldKeys = updated;
    inputDirty = true;
  }
  ...
  private void addInteractHandler() {
    this.gpanel.setInteractableHandler(
        new AbstractInteractableHandler() {
