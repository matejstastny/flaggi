...
public class GameManager implements Closeable, Updatable {
    ...
    public void update() {
        ...
        ClientStateUpdate outgoing = ClientStateUpdate.newBuilder()
                .setPlayerUuid(uuid)
                .setGameUuid(gameUuid)
                .setClientMouseInput(ClientMouseInput.newBuilder()
                        .setX(worldMouseX)
                        .setY(worldMouseY)
                        .build())
                .setClientKeyInput(
                        ClientKeyInput.newBuilder().addAllHeldKeys(keySnapshot).build())
                .build();
    ...
    private static KeyEvent makeSyntheticShootEvent() {
        return new KeyEvent(
                new java.awt.Component() {}, KeyEvent.KEY_PRESSED, 0, 0, KeyEvent.VK_F, KeyEvent.CHAR_UNDEFINED);
    }
}
