...
public class UdpManager implements Runnable {
    ...
    public UdpManager() {
        try {
            ...
            Logger.log(
                    LogLevel.DBG, "Datagram socket created on " + addr.getHostAddress() + ":" + socket.getLocalPort());
        ...
        while (!Thread.currentThread().isInterrupted()) {
            try {
                ...
                ServerStateUpdate update =
                        ServerStateUpdate.parseFrom(new ByteArrayInputStream(packet.getData(), 0, packet.getLength()));
