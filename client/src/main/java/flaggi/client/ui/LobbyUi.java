...
public class LobbyUi extends Renderable implements Scrollable, Interactable, Updatable {
    ...
    private void renderClientItem(Graphics2D g, ClientItem clientItem, int offsetY) {
        ...
        g.setColor(Color.WHITE);
        int[] namePos = FontUtil.getCenteredPosition(width, height, g.getFontMetrics(), clientItem.name);
    ...
    private void renderJoinButton(Graphics2D g, int itemWidth, int offsetY) {
        ...
        RoundRectangle2D button = new RoundRectangle2D.Float(
                itemWidth - buttonWidth - padding, offsetY + padding, buttonWidth, buttonHeight, radius, radius);
        g.fill(button);

        int[] textPos = FontUtil.getCenteredPosition(buttonWidth, buttonHeight, g.getFontMetrics(), "Join");
        g.setColor(Color.WHITE);
