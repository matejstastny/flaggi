...
public class GameUi extends Renderable {
  ...
  private static final double PLAYER_HB_OX = -2.5,
      PLAYER_HB_OY = -2.5,
      PLAYER_HB_W = 5,
      PLAYER_HB_H = 5;
    ...
    try {
      this.floor =
          ImageUtil.createTiledImage(
              ImageUtil.getImageFromResource("sprites/floor-tile/floor-tile.png"),
              roomSize[0],
              roomSize[1]);
    } catch (IOException e) {
  ...
  private void renderPlayer(ServerGameObject o, VhGraphics g, Container root) {
    Sprite sprite =
        playerSprites.computeIfAbsent(
            o.getUsername(), k -> new Sprite(skinToFolder(o.getSkin()), SPRITE_FPS));
