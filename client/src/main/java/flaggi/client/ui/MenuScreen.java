...
public class MenuScreen extends Renderable implements Interactable, Typable {
  ...
  public MenuScreen(
      String initName, String initIp, BiFunction<String, String, String> buttonPressAction) {
  ...
  private void renderErrorMessage(Graphics2D g) {
    ...
    int[] errorPos =
        FontUtil.getCenteredPosition(px(100), 0, g.getFontMetrics(), this.errorMessage);
  ...
  private void renderTextFields(Graphics2D g, Container fcra) {
    ...
    Image scaledTextField =
        ImageUtil.scaleImage(this.textField, nameField.width, nameField.height, false);
    g.drawImage(scaledTextField, nameField.x, nameField.y, fcra);
    g.drawImage(scaledTextField, ipField.x, ipField.y, fcra);

    renderTextField(
        g, "Name: " + usernameInput, isNameFieldFocused, nameField.x + px(3), nameField.y + px(6));
  ...
  private void renderStartButton(Graphics2D g, Container fcra) {
    ...
    Image scaledButton =
        ImageUtil.scaleImage(this.button, startButton.width, startButton.height, false);
    g.drawImage(scaledButton, startButton.x, startButton.y, fcra);
    g.setColor(Color.WHITE);
    int[] startButtonTextPos =
        FontUtil.getCenteredPosition(
            startButton.width, startButton.height, g.getFontMetrics(), "START");
    if (!buttonActive) {
      g.drawString(
          "START", startButton.x + startButtonTextPos[0], startButton.y + startButtonTextPos[1]);
    }
  }
  ...
  private String getInteractedElement(MouseEvent e) {
    ...
    if (getStartButtonBounds().contains(x, y)) return "start_button";
    if (getNameFieldBounds().contains(x, y)) return "name_field";
    if (getIpFieldBounds().contains(x, y)) return "ip_field";
  ...
  public void type(KeyEvent e) {
    ...
    if (isNameFieldFocused) handleTyping(e, true);
    else if (isIpFieldFocused) handleTyping(e, false);
  }

  // Public -------------------------------------------------------------------

  /**
   * Resets the menu screen to its initial state. This method clears the error, and resets the
   * button state.
   */
  public void reset() {
    this.errorMessage = "";
  ...
  private void handleTyping(KeyEvent e, boolean isNameField) {
    ...
    if ((isNameField && (Character.isLetterOrDigit(c) || Character.isWhitespace(c)))
        || (!isNameField && (Character.isLetterOrDigit(c) || c == '.' || c == ':'))) {
      input.append(c);
  ...
  private void loadFont() {
    try {
      this.font =
          FontUtil.createFontFromResource("fonts/PixelifySans-VariableFont_wght.ttf")
              .deriveFont(Font.PLAIN, 25);
    } catch (IOException | FontFormatException e) {
      Logger.log(LogLevel.WRN, "MenuScreen: Font cannot be loaded.", e);
  ...
  private void loadImages() {
    try {
      ...
      Logger.log(LogLevel.WRN, "Couldn't load " + getClass().getSimpleName() + " textures.", e);
    }
  }
