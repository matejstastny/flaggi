...
# ───────────────────────────────────────────────────────────────────────────────
# DMG Creation Script for "Flaggi"
# Builds a Java package
# ───────────────────────────────────────────────────────────────────────────────
...
handle_options() {
    ...
    if [ "$mode_count" -gt 1 ]; then
        echo "Error: Cannot specify multiple modes (client, server, docker, editor) at once." >&2
        log "Error: Cannot specify multiple modes (client, server, docker, editor) at once."
        usage
    fi
}
...
build_minimal_jre() {
    ...
    echo "Creating minimal JRE..." | tee -a "$LOG_FILE"

    jlink \
        --add-modules java.base,java.desktop \
        --output "$DIET_JRE" \
        --no-header-files 2>>"$LOG_FILE"

    echo "JRE created at $DIET_JRE" | tee -a "$LOG_FILE"
}
...
check_java_version() {
    ...
    if [[ "$java_version" != "1.8.0" ]]; then
        echo "Error: Java 8 is required to run this script. Current version: $java_version" >&2
        log "Error: Java 8 is required to run this script. Current version: $java_version"
        exit 1
    fi
}
...
build_and_run() {
    ...
    # Build JAR
    echo "Building the $app_name JAR..." | tee -a "$LOG_FILE"
    cd "$app_dir"
    ./gradlew shadowjar 2>>"$LOG_FILE"

    # Run the JAR
    if [ -f "$jar_file" ]; then
        if [ "$use_diet_jre" = true ]; then
            echo "Making the diet JRE ..." | tee -a "$LOG_FILE"
            cd "$SCRIPT_DIR"
            build_minimal_jre
            echo "Running the $app_name using diet JRE..." | tee -a "$LOG_FILE"
            cd "$(dirname "$jar_file")"
            print_divider
            echo ""
            "$DIET_JRE/bin/java" -jar "$(basename "$jar_file")" 2>>"$LOG_FILE" || log "Failed to run $app_name using diet JRE"
        else
            echo "Running the $app_name using global JRE..." | tee -a "$LOG_FILE"
            cd "$(dirname "$jar_file")"
            echo ""
            print_divider
            java -jar "$(basename "$jar_file")" 2>>"$LOG_FILE" || log "Failed to run $app_name using global JRE"
        fi
    else
...
run_docker() {
    ...
    # Build server JAR
    echo "Building the server JAR..." | tee -a "$LOG_FILE"
    cd "$APP_DIRS_SERVER"
    ./gradlew shadowjar 2>>"$LOG_FILE"
    ...
    docker build -t flaggi-server . 2>>"$LOG_FILE"

    # Stop and remove any existing container with the same name
    if [ "$(docker ps -aq -f name=flaggi-server)" ]; then
        docker stop flaggi-server 2>>"$LOG_FILE"
        docker rm flaggi-server 2>>"$LOG_FILE"
    fi

    # Run the docker container & expose the ports used by the server
    echo "Running the Docker container..." | tee -a "$LOG_FILE"
    docker run \
        --name flaggi-server \
        -p 54321:54321/tcp \
        -p 54322:54322/udp \
        -e HOST_IP=$host_ip \
        flaggi-server 2>>"$LOG_FILE"
}
...
if [ -z "$selected_mode" ]; then
    echo "❌ Error: No mode specified. Use 'client', 'server', 'docker', or 'editor'." >&2
    log "Error: No mode specified. Use 'client', 'server', 'docker', or 'editor'."
    usage
fi
