...
public final class FontUtil {
  ...
  private FontUtil() {
    throw new UnsupportedOperationException(
        "FontUtil is a utility class and cannot be instantiated.");
  }
   ...
   */
  public static Font createFontFromResource(String resourcePath)
      throws IOException, FontFormatException {
    try (InputStream fontStream =
        FontUtil.class.getClassLoader().getResourceAsStream(resourcePath)) {
      if (fontStream == null) {
   ...
   * @return An {@code int[]} where index 0 represents {@code x} and index 1 represents {@code y}
   *     positions.
   */
  public static int[] getCenteredPosition(int width, int height, FontMetrics fm, String text) {
    return new int[] {
      (width - fm.stringWidth(text)) / 2, (height - fm.getHeight()) / 2 + fm.getAscent()
    };
  }
   ...
   * @param alignRight If {@code true}, aligns text to the right; otherwise, aligns to the left.
  ...
  /**
   * Calculates the origin point to render the text bottom-right of the given point.
   *
   * @param fm The {@code FontMetrics} object to get text dimensions.
   * @param text The text to be rendered.
   * @param point The reference point as an {@code int[]} of length 2.
  ...
  /**
   * Calculates the origin point to render the text bottom-left of the given point.
   *
   * @param fm The {@code FontMetrics} object to get text dimensions.
   * @param text The text to be rendered.
   * @param point The reference point as an {@code int[]} of length 2.
   ...
   * @throws IllegalArgumentException if the array is {@code null} or has less than two elements.
   */
