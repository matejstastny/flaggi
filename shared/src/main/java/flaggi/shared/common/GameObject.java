...
public class GameObject {
	...
	public ServerGameObject toProto() {
		return ServerGameObject.newBuilder().setType(type).setX(x).setY(y).setCollX(collision.getX()).setCollY(collision.getY()).setCollWidth(collision.getWidth()).setCollHeight(collision.getHeight()).setHp(hp).build();
	}

	/**
	 * Returns true if this object's world-space hitbox overlaps with
	 * {@code other}'s. Hitbox x/y are offsets from the object's origin.
	 */
