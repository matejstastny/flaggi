...
/**
 * Logger class for logging messages with different log levels. Supports logging to console with
 * colored output and to a file.
 */
public class Logger {
  ...
  public static void setLogLevelsToIgnore(LogLevel... logLevels) {
    ...
    log =
        log.isEmpty()
            ? "Logging enabled for all levels"
            : "Ignored levels: " + log.substring(0, log.length() - 2);
  ...
  public static void log(LogLevel level, String message, Exception e) {
    ...
    String paddedLevel =
        String.format("%-" + (getMaxLogLevelLength() + 2) + "s", "[" + level.name() + "]");
    String logMessage =
        String.format(
            "%s%s %s%s %s %s",
            DATE_COLOR, timestamp, level.getColor(), paddedLevel, TermColors.WHITE, message);
