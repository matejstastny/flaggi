...
public class MapData {
  ...
  public synchronized void setSpawnpoints(
      int spawnpoint1X, int spawnpoint1Y, int spawnpoint2X, int spawnpoint2Y) {
  ...
  private void isInBounds(int x, int y) {
    if (x < 0 || x > width || y < 0 || y > height) {
      throw new IllegalArgumentException(
          String.format(
              "Coordinates (%d, %d) are outside map bounds (%d x %d)", x, y, width, height));
    }
  }
  ...
  public enum ObjectType {
    ...
    public static ObjectType fromName(String name) {
      return Arrays.stream(values())
          ...
          .findFirst()
          .orElseThrow(() -> new IllegalArgumentException("Unknown ObjectType: " + name));
    }
  }
  ...
  public static class Spawnpoint {
    ...
    public String toString() {
      return String.format(
          "Spawnpoint{oneX=%d, oneY=%d, twoX=%d, twoY=%d}", oneX, oneY, twoX, twoY);
    }
  }
}
