// ------------------------------------------------------------------------------
// BulletGameObject.java - Bullet game object with velocity and ownership
// ------------------------------------------------------------------------------
...
public class BulletGameObject extends GameObject {
	 ...
	 */
	public BulletGameObject(double x, double y, double targetX, double targetY, String ownerUuid, Hitbox hitbox) {
	...
	public ServerGameObject toProto() {
		return ServerGameObject.newBuilder().setType(GameObjectType.BULLET).setX(x()).setY(y()).setCollX(collision().getX()).setCollY(collision().getY()).setCollWidth(collision().getWidth()).setCollHeight(collision().getHeight()).build();
	}

	// Public -------------------------------------------------------------------

	/**
	 * Advance bullet position by one tick. Returns true if the bullet is still
	 * alive.
	 */
	public boolean tick(int mapWidth, int mapHeight) {
		...
		return lifetimeTicks < MAX_LIFETIME_TICKS && x() >= 0 && x() <= mapWidth && y() >= 0 && y() <= mapHeight;
	}
	...
	/**
	 * World-space axis-aligned bounding box for collision checks. The hitbox
	 * offsets (collision().getX/Y()) are relative to the object origin.
	 */
