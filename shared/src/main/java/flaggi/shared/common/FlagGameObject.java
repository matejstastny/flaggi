...
public class FlagGameObject extends GameObject {
	...
	public ServerGameObject toProto() {
		return ServerGameObject.newBuilder().setType(GameObjectType.FLAG).setX(x()).setY(y()).setCollX(collision().getX()).setCollY(collision().getY()).setCollWidth(collision().getWidth()).setCollHeight(collision().getHeight()).build();
	}
